...
&
    \ensuremath{\mathrm{1}\mathbin{+}\mathbb{Z}\times[\mathbb{Z}]}
            \ar[r]^-{\ensuremath{\Varid{id}+\langle\langle\Varid{head},\Varid{id}\rangle,\Varid{tail}\comp \Varid{cons}\rangle}}
&
    \ensuremath{\mathrm{1}\mathbin{+}(\mathbb{Z}\times[\mathbb{Z}])\times[\mathbb{Z}]}
            \ar[d]^-{\ensuremath{\Varid{id} + (\Varid{id} \times \Varid{id}) \times (\ana{\Varid{divide}})}}              
\\
    \ensuremath{[\mathbb{Z}\times[\mathbb{Z}]]}
            \ar@/^/[rrr]^-{\ensuremath{\Varid{out}}}
            \ar[d]_-{\ensuremath{\cata{\Varid{conquer}}}}
&
&
&
    \ensuremath{\mathrm{1}\mathbin{+}(\mathbb{Z}\times[\mathbb{Z}])\times[\mathbb{Z}\times[\mathbb{Z}]]}
            \ar@/^/[lll]_-{\ensuremath{\Varid{in}}}
            \ar[d]^-{\ensuremath{\Varid{id} + (\Varid{id} \times \Varid{id}) \times (\cata{\Varid{conquer}})}}
\\
    \ensuremath{\mathbb{Z}\times[\mathbb{Z}]}
&
&
&
    \ensuremath{\mathrm{1}\mathbin{+}(\mathbb{Z}\times[\mathbb{Z}])\times(\mathbb{Z}\times[\mathbb{Z}])}
            \ar[lll]^-{\ensuremath{\alt{\langle\Varid{const}\ 0,\Varid{nil}\rangle\ }{(\uncurry{\Varid{\char38\char38}})\comp \langle\Varid{pp1},\Varid{pp2}\rangle\Varid{->}\pi{1},\pi{2}}}}             
}
\end{eqnarray*}

\ensuremath{\Varid{pp1}\;\mathrel{=}\;(\uncurry{\Varid{/=}})\comp\langle\pi{1}\comp\pi{2},\Varid{const}\ 0\rangle}

\ensuremath{\Varid{pp2}\;\mathrel{=}\;(\uncurry{\Varid{<=}})\comp\langle\pi{1}\comp\pi{2},\Varid{length}\comp\pi{2}\comp\pi{2}\rangle}
...
\column{B}{@{}>{\hspre}l<{\hspost}@{}}%
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
\>[B]{}\Varid{outExpr}\;(\Varid{V}\;\Varid{a})\mathrel{=}i_1\;\Varid{a}{}\<[E]%
\\
\>[B]{}\Varid{outExpr}\;(\Varid{N}\;\Varid{b})\mathrel{=}i_2\;(i_1\;\Varid{b}){}\<[E]%
\\
\>[B]{}\Varid{outExpr}\;(\Varid{T}\;\Varid{op}\;\Varid{l})\mathrel{=}i_2\;(i_2\;(\Varid{op},\Varid{l})){}\<[E]%
\\
\\[\blanklineskip]%
...
\column{B}{@{}>{\hspre}l<{\hspost}@{}}%
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
\>[B]{}\Varid{let\char95 exp}\mathrel{=}\Varid{flip}\;(\mathrel{>}\mathrel{>}\mathrel{=}) {}\<[E]%
\ColumnHook
\end{hscode}\resethooks

\begin{eqnarray*}
\xymatrix@C=5cm@R=2cm{
    \ensuremath{\Varid{Expr}\;\Varid{C}\;\Varid{A}}
            \ar[d]_-{\ensuremath{\Varid{let\char95 exp}\;\Varid{f}}}
            \ar[r]^-{\ensuremath{\Varid{outExpr}}}
&
    \ensuremath{\Varid{A}\mathbin{+}(\Varid{C}\mathbin{+}\Varid{OP}\times(\Varid{Expr}\;\Varid{C}\;\Varid{A}))}
            \ar[d]^-{\ensuremath{\Varid{id}\mathbin{+}(\Varid{id}\mathbin{+}\Varid{id}\times(\Varid{map}(\Varid{let\char95 expr}\;\Varid{f}))}}
\\
    \ensuremath{\Varid{Expr}\;\Varid{C}\;\Varid{B}}
&
    \ensuremath{\Varid{A}\mathbin{+}(\Varid{C}\mathbin{+}\Varid{OP}\times(\Varid{Expr}\;\Varid{C}\;\Varid{B}))}
            \ar[l]^-{\ensuremath{\alt{\Varid{f}}{\alt{\Varid{N}}{\uncurry{\Conid{T}}}}}}
}
\end{eqnarray*}
